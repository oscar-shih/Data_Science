...
def Sort(sub_li):
    ...
    sub_li.sort(reverse=True, key = lambda x: abs(x[1]))
    return sub_li
...
labels = []
index = []
with open(join(data_dir, "label.txt"), "r") as f:
    for line in f:
        if line != "\n":
            labels.append(int(line))

            
with open(join(data_dir, "index.txt"), "r") as f:
    for line in f:
        if line != "\n":
            index.append(line)
            

genes = pd.read_csv(join(data_dir, "gene.txt"), " ", names=labels).T
indexs = []
...
rwfs = RandomWalkFeatureSelection(
    model, 
    cv, 
    n_steps=2000, 
...
rwfs.fit(genes, indexs, verbose=0)

count = 0
useful_feature_idx = []
with open("p2.txt", "w") as f:
    
    for feature in rwfs.best_features_:
        if feature:
            useful_feature_idx.append(count)
            f.writelines(f"Index {count} \t {index[count]}")
        count += 1