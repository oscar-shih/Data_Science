...
def loss1(usersPerCircle, usersPerCircleP):
    ...
    for i in range(psize):
        for j in range(psize):
            ...
            # Compute the edit distance between the two circles
            nedits = len(circle.union(circleP)) - \
                len(circle.intersection(circleP))
            mm[i][j] = nedits
            mm2[i][j] = nedits
    ...
    if psize == 0:
        ...
        # Compute the optimal alignment between predicted and groundtruth circles
...
for gl, pl in zip(gLines, pLines):
    ...
    circlesG[int(uidG)] = [set([int(x) for x in c.split()])
                           for c in friendsG.split(';')]
    circlesP[int(uidP)] = [set([int(x) for x in c.split()])
                           for c in friendsP.split(';')]
